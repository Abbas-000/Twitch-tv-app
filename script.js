  ...
  while (i < allChannels.length) {
    ...
    $.getJSON(
      "https://wind-bow.gomix.me/twitch-api/streams/" +
        ...
        "?callback=?",
      function(streamData) {
        /*the testName var is only to get the channels name to paas it with an argument in the request url if the channel's stream is null*/
        var testName = streamData._links.channel.substring(38);
        ...
        if (streamData.stream != null) {
          ...
          var status = streamData.stream.channel.game;
          //creating the <li> begins from here
          var media =
            "<li class='media my-4'><img class='mr-3' src=" +
            logo +
            " alt='channel's logo'><div class='media-body text-center'><a href=" +
            link +
...
function isOnline(requestUrl) {
  $.getJSON(requestUrl, function(channelData) {
    ...
    var link = channelData.url;
    var media = "<li class='media my-4'><img class='mr-3' src=" + logo + " alt='channel's logo'><div class='media-body text-center'><a href=" + link + " target='_blank'><h5 class='mt-0 mb-1'>" + name + " <sup><span class='badge badge-pill badge-warning'>Offline</span></sup></h5></a></div></li>";
